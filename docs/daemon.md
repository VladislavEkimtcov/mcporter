...
Proxy loss also ends the associated MCP connection, even if its child process remains alive. The daemon retires that exact child through its existing serialized cleanup before the next explicit request creates a fresh authenticated proxy and child. An interrupted call is returned as a failure and is never replayed; its browser action may already have happened, so check the page before repeating it. The one-use proxy port is never reopened. Ordinary MCP application errors leave a healthy connection intact.
...
Choose shared Chrome routing with `chromeDevtoolsRelay` (or `chrome_devtools_relay`) on the canonical global server definition. Its `env` map may also set `MCPORTER_CHROME_DEVTOOLS_RELAY_POLICY`:
...
```jsonc
{
  "mcpServers": {
    "chrome-devtools": {
      ...
      "env": { "MCPORTER_CHROME_DEVTOOLS_RELAY_TIMEOUT_MS": "20000" },
    },
  },
}
```

Before probing, MCPorter runs the non-secret `openclaw browser extension cdp --json` metadata command without the legacy-bearer option. POSIX launches `openclaw` directly. Windows resolves an existing `openclaw.cmd` only from safe absolute `PATH` entries, rejects the current directory and command-expanding path bytes, then passes the quoted absolute shim plus constant arguments to a validated system `cmd.exe`. Process spawning always keeps `shell: false`; time and combined output are bounded, and timeout or overflow terminates the full Windows process tree through the validated system `taskkill.exe`. Discovery diagnostics never include stdout or stderr. MCPorter accepts only the exact loopback Browser Relay Authentication v2 CDP contract whose `keyId` matches the local relay credential. An explicit nonempty `MCPORTER_CHROME_DEVTOOLS_RELAY_URL` remains authoritative and skips metadata discovery; unavailable, timed-out, malformed, unsafe, or incompatible metadata returns `relay_discovery_failed` with a reason such as `discovery-timeout`. MCPorter does not guess port `18799`, probe another endpoint, or turn failed discovery into a browser-owner conflict. A later explicit call retries discovery; an established owner is retained and must still validate before dispatch. Relay control and OpenClaw discovery variables in a server definition's `env` map use the same placeholder resolution as its runtime transport and override process-level values for both routing and daemon identity.

Metadata discovery and the authenticated `/json/version` probe each use the relay timeout, which is 20 seconds by default to accommodate OpenClaw CLI cold starts. `MCPORTER_CHROME_DEVTOOLS_RELAY_TIMEOUT_MS` accepts milliseconds, clamped to 100–30000; unset, zero, non-integer, and otherwise invalid values use 20000. Credential discovery follows OpenClaw: `OPENCLAW_OAUTH_DIR` wins, then `OPENCLAW_STATE_DIR`; otherwise credentials live under the effective home in `.openclaw/credentials`, or `.openclaw-<profile>/credentials` for a validated non-default `OPENCLAW_PROFILE`. Explicit `OPENCLAW_HOME` takes precedence over `HOME` and `USERPROFILE`.
...
### Persistent relay settings and daemon restarts

For shared Chrome, shell exports of `MCPORTER_CHROME_DEVTOOLS_RELAY_URL`, `MCPORTER_CHROME_DEVTOOLS_RELAY_TIMEOUT_MS`, relay policy, and `OPENCLAW_*` controls do not change an already-running daemon. At startup the daemon deliberately removes these inherited controls before resolving canonical Chrome configuration, so setting them on the launching call also does not configure a replacement daemon. This prevents whichever caller starts the shared host from changing its browser route.

Set persistent controls as literal values in the existing Chrome definition’s `env` map in `~/.mcporter/mcporter.json` (or `.jsonc` when no `.json` exists), alongside `chromeDevtoolsRelay: "require"`. The example above survives respawns from other shells and GUI clients. Keep the rest of the existing definition intact. Prefer metadata discovery; if an explicit endpoint is necessary, set `MCPORTER_CHROME_DEVTOOLS_RELAY_URL` there to its HTTP loopback base URL, for example `http://127.0.0.1:28799`, rather than a WebSocket `/cdp` URL. A shell-variable placeholder for these controls is unsuitable because that variable is removed from the canonical startup environment.

After a configuration change, wait for active calls to finish, run `mcporter daemon stop`, and confirm it succeeds before the next call. If stop refuses active calls, wait and retry it. The next invocation starts a host with the updated canonical configuration. A successful discovery of a genuinely changed endpoint or credential still requires explicit owner retirement; a discovery timeout alone does not.

Chrome auto-connect `list` and `call` commands default to a 300-second outer deadline. The initial flow can run five separate metadata discoveries plus an authenticated relay probe, each with a maximum 30-second budget, in addition to daemon readiness and MCP startup. The larger outer deadline accommodates these steps while preserving fresh ownership checks before dispatch. Other servers keep their 30-second list and 60-second call defaults. Explicit `--timeout`, `MCPORTER_LIST_TIMEOUT`, and `MCPORTER_CALL_TIMEOUT` values still win; choose enough time for the whole operation. This outer deadline is separate from the canonical per-step relay timeout.
